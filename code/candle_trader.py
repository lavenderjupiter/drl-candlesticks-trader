...
HEIGHT, WIDTH, N_CHANNELS = 224, 224, 3 
...
VIDEO_DIR = make_path(INIT_PATHES['video'])
...
class train:
    def __init__(self, device, name):
        ...
        self.message = self.name + '(a)'
        self.backbone = models.AddCBAM(input_shape=(N_CHANNELS, HEIGHT, WIDTH), 
    ...
    def run(self, count, train_set, episodes=50, run_checkpoint=False):
        ...
        env = environ.BitcoinEnv(train_set, image_shape=(N_CHANNELS, HEIGHT, WIDTH), balance=INITIAL_ACCOUNT_BALANCE,
                                          threshold=LEARN_THRESHOLD, commission_perc=COMMISSION_PERCENT)

        print(self.name + '@' + timestamp)
        if self.name.__contains__('dqn'):
            if self.name == 'double-dqn':
                agent = agents.DoubleDQN(env, self.backbone, INITIAL_ACCOUNT_BALANCE, HYPERPARAMS, acc_feats=ACCOUNT_FEATURES, device=self.device)
            elif self.name == 'dueling-dqn':
                agent = agents.DuelingDQN(env, self.backbone, INITIAL_ACCOUNT_BALANCE, HYPERPARAMS, acc_feats=ACCOUNT_FEATURES, device=self.device)
            else:
                agent = agents.DQN(env, self.backbone, INITIAL_ACCOUNT_BALANCE, HYPERPARAMS, acc_feats=ACCOUNT_FEATURES, device=self.device)
            ...
            agent = agents.PPO(env, self.backbone, INITIAL_ACCOUNT_BALANCE, HYPERPARAMS, acc_feats=ACCOUNT_FEATURES, device=self.device)
            if run_checkpoint:
...
class valid:
    ...
    def run(self, count, valid_set):
        message = self.message + '_agent' + str(count) # dqn(a)_agent0
        ...
        env = environ.BitcoinEnv(valid_set, image_shape=(N_CHANNELS, HEIGHT, WIDTH), balance=INITIAL_ACCOUNT_BALANCE,
                                 threshold=1, commission_perc=COMMISSION_PERCENT)

        if self.name == 'dqn' or self.name == 'double-dqn':
            net = models.DQNNet(env.observation_space.shape, env.action_space.n, self.backbone, account_feats=len(ACCOUNT_FEATURES))
        elif self.name == 'dueling-dqn':
            net = models.DuelingNet(env.observation_space.shape, env.action_space.n, self.backbone, account_feats=len(ACCOUNT_FEATURES))
        elif self.name == 'ppo':
            net = models.ActorPPO(env.observation_space.shape, env.action_space.n, self.backbone, account_feats=len(ACCOUNT_FEATURES))
        params = torch.load(model_path, map_location='cpu')  #dqn(a)_tc0_agent0_best.pth
        ...
        with open(log_filename, 'a') as log:
            log.truncate(0)
            state = env.reset()
            image, ratio, gain, position, fund_rate = state
            ...
            with torch.no_grad():
                while True:
                    ...
                    if self.name == 'ppo':
                        ...
                        dist = net(image_t, account_t, action_mask)
                        ...
                        q_values = net(image_t, account_t)
                        q_values[action_mask == 0] = -9999999
                        action = np.argmax(q_values.data.numpy()[0])
                    c[action] += 1
                    actions.append(action)
                    [image, ratio, gain, position, fund_rate], _, done, info = env.step(action)
...
class test:
    def __init__(self, name, datetime):
        self.name = name.lower().replace(' ', '')
        self.datetime = datetime.split('_')[0]
        self.test_path = make_path(TEST_RESULT_DIR + self.datetime + '/')
        self.test_log_path = make_path(TEST_RESULT_DIR + self.datetime + '/log/')
        self.message = self.name + '(a)'
        self.backbone = models.AddCBAM(input_shape=(N_CHANNELS, HEIGHT, WIDTH), 
                                       backbone=nn.Sequential(*( list(RESNET.children())[:-2])))
    ...
    def run(self, test_set, record=False):
        ...
        env = environ.BitcoinEnv(test_set, image_shape=(N_CHANNELS, HEIGHT, WIDTH), balance=INITIAL_ACCOUNT_BALANCE,
                                 threshold=1, commission_perc=COMMISSION_PERCENT)
        ...
        with open(log_filename, 'a') as log:
            ...
            image, ratio, gain, position, fund_rate = state
            c = collections.Counter()
            video_recorder = VideoRecorder(env, VIDEO_DIR + self.message + '_' + self.datetime + '.mp4', enabled=record)
            current_step = 0
            ...
            while True:
                ...
                [image, ratio, gain, position, fund_rate], _, done, info = env.step(ensemble_action)
    ...
    def baseline(self, image_test_set, df_test_set):
        baseline_model = Baseline(HYPERPARAMS, self.test_path, INITIAL_ACCOUNT_BALANCE, COMMISSION_PERCENT, FPS)
        if self.name == 'rp':
            baseline_model.randomized_policy(image_test_set)
        elif self.name == 'bh':
            baseline_model.buy_and_hold(image_test_set)
        elif self.name == 'hp':
            baseline_model.range_break(df_test_set)
